...
mod tests {
    ...
    async fn test_ingest_redis() -> anyhow::Result<()> {
        ...
        let response = http_client.get::<String>(health_url).await?;
        ...
        let response = http_client
            .post::<String, Vec<u8>>(ingest_url, Some(body))
            .await?;
        let status = response.status();
        assert_eq!(StatusCode::OK, status);
        // wait for complete
        ...
        let response = http_client
            .post::<String, Vec<u8>>(batch_ingest_url, Some(body))
            .await?;
        let status = response.status();
        assert_eq!(StatusCode::OK, status);
        // wait for complete
        ...
        let response = http_client
            .post::<String, String>(shutdown_url, None)
            .await?;
        let status = response.status();
        assert_eq!(StatusCode::OK, status);
