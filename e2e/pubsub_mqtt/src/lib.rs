...
mod tests {
    ...
    #[tokio::test]
    async fn test_pubsub_mqtt() -> anyhow::Result<()> {
        // setup clients
        ...
        assert_eq!(0, context.get_failure_run());
        // amqp publisher
        let context_path = append_path(CONTEXT_SERVER_PIPE, "mqtt_publisher");
        let context_url = build_url(CONTEXT_SERVER_ADDRESS, &context_path);
        let context = http_client
            .get_json::<String, PipeContext>(context_url)
            .await?;
        assert_eq!("mqtt_publisher", context.get_name());
        assert_eq!("receive", context.get_state());
        assert_eq!(1, context.get_total_run());
        assert_eq!(0, context.get_failure_run());
        // mqtt subscriber
        let context_path = append_path(CONTEXT_SERVER_PIPE, "mqtt_subscriber");
        let context_url = build_url(CONTEXT_SERVER_ADDRESS, &context_path);
        let context = http_client
            .get_json::<String, PipeContext>(context_url)
            .await?;
        assert_eq!("mqtt_subscriber", context.get_name());
        assert_eq!("receive", context.get_state());
        assert_eq!(1, context.get_total_run());
        assert_eq!(0, context.get_failure_run());
        // avro deser pipe
