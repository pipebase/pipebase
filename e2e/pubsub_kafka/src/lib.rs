...
mod tests {
    ...
    async fn test_pubsub_kafka() -> anyhow::Result<()> {
        ...
        assert_eq!(0, context.get_failure_run());
        // kafka producer
        let context_path = append_path(CONTEXT_SERVER_PIPE, "kafka_producer");
        let context_url = build_url(CONTEXT_SERVER_ADDRESS, &context_path);
        let context = http_client
            .get_json::<String, PipeContext>(context_url)
            .await?;
        assert_eq!("kafka_producer", context.get_name());
        assert_eq!("receive", context.get_state());
        assert_eq!(1, context.get_total_run());
        assert_eq!(0, context.get_failure_run());
        // kafka consumer
        let context_path = append_path(CONTEXT_SERVER_PIPE, "kafka_consumer");
        let context_url = build_url(CONTEXT_SERVER_ADDRESS, &context_path);
        let context = http_client
            .get_json::<String, PipeContext>(context_url)
            .await?;
        assert_eq!("kafka_consumer", context.get_name());
        assert_eq!("receive", context.get_state());
        assert_eq!(1, context.get_total_run());
        assert_eq!(0, context.get_failure_run());
        // avro deser pipe
